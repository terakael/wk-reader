...
// Target mix: ~10 nouns, ~3 suru-nouns, ~5 verbs, ~4 adjectives, ~2 adverbs,
// plus up to 3 wildcards sampled from ranks 20–80.
function retrieveVocab(queryVec, maxLevel) {
  ...
  // Wildcards: random sample from overall ranks 20–80, skip numbers + proper nouns
  const wildcardPool = scores
    .slice(20, 80)
    .filter(s => !picked.has(s.r) && s.pos !== "number");
  while (result.length < 27 && wildcardPool.length > 0) {
